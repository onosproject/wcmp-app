...
local-deps: local-helmit local-onos-api local-onos-lib-go local-onos-test local-onos-topo
...
wcmp-app-docker: mod-update local-deps # @HELP build wcmp-app base Docker image
	docker build --platform linux/amd64 . -f build/wcmp-app/Dockerfile \
		-t ${DOCKER_REPOSITORY}wcmp-app:${WCMP_APP_VERSION}
...
kind-only:
	...
	kind load docker-image --name ${KIND_CLUSTER_NAME} ${DOCKER_REPOSITORY}wcmp-app:${ONOS_CONFIG_VERSION}
...
clean:: # @HELP remove all the build artifacts
	rm -rf ./build/_output ./vendor ./cmd/wcmp-app/wcmp-app ./cmd/onos/onos
	go clean -testcache github.com/onosproject/wcmp-app/...
